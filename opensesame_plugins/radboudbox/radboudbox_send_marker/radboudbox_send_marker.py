...
class RadboudboxSendMarker(Item):
    ...
    def run(self):
        if self.dummy_mode == 'no':
            self.set_item_onset()
            self.experiment.radboudbox.sendMarker(val=(ord('M')))
...
class QtRadboudboxSendMarker(RadboudboxSendMarker, QtAutoPlugin):

    def __init__(self, name, experiment, script=None):
        RadboudboxSendMarker.__init__(self, name, experiment, script)
